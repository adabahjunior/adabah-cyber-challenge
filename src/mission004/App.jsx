...
import { MISSION_004 } from "../mission/missions/mission004";
import LoginLogViewer from "./components/LoginLogViewer";
...
const STORAGE_KEY = "acc_mission_004_v1";
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      setMissionStatus(
        saved.missionStatus ||
          (saved.completed ? "AUTHENTICATION RESTORED" : saved.started ? "INVESTIGATING" : "STANDBY")
      );
    } catch (_) {}
  }, []);
  ...
  function completeMission(verifiedMap = verified) {
    ...
    setCompleted(true);
    setMissionStatus("AUTHENTICATION RESTORED");
      ...
      {showCompleteAnim ? (
        <div className="complete-overlay">
          <div className="complete-burst">
            ...
            <h2>CASE STATUS · AUTHENTICATION RESTORED</h2>
            <p className="mono">THE BREACHED VAULT</p>
          </div>
        </div>
      ) : null}

      <div className="mission-app mission-002 mission-004">
        ...
        <section className="glass section" style={{ padding: "1.25rem 1.3rem", marginTop: "1rem" }}>
          ...
          {!started ? (
            <p className="mono dim" style={{ marginTop: "0.75rem", fontSize: "0.82rem" }}>
              Click Access Authentication Server to begin the investigation.
            </p>
          ) : (
          ...
          <div className="m2-main stack">
            <section className="section">
              <div className="row" style={{ justifyContent: "space-between", marginBottom: "0.85rem" }}>
                <div>
                  <p className="eyebrow">Authentication Server</p>
                  <h2 style={{ fontSize: "1.15rem" }}>Breach evidence modules</h2>
                </div>
              ...
              {started && activeEv ? (
                <div className="glass evidence-stage" style={{ marginTop: "1rem", padding: "1.1rem" }}>
                  ...
                  {activeEv.type === "logs" && <LoginLogViewer />}
                  {activeEv.type === "vault" && <PasswordVault />}
                  {activeEv.type === "hashes" && <HashLaboratory />}
                  {activeEv.type === "policy" && <PolicyViewer />}
                </div>
              ) : null}
            </section>
            ...
            {completed && completionMeta ? (
              <section className="glass" style={{ padding: "1.25rem 1.3rem" }}>
                ...
                <p className="mono" style={{ color: "var(--red-bright)", marginTop: "0.35rem" }}>
                  CASE STATUS · Authentication Restored
                </p>
