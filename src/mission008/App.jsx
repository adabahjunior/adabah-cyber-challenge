...
import { MISSION_008 } from "../mission/missions/mission008";
import FileExplorer from "./components/FileExplorer";
import BehaviourReport from "./components/BehaviourReport";
...
const STORAGE_KEY = "acc_mission_008_v1";
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      setMissionStatus(
        saved.missionStatus ||
          (saved.completed ? "MALWARE CONTAINED" : saved.started ? "CONTAINING" : "STANDBY")
      );
    } catch (_) {}
  }, []);
  ...
  const threatLevel = useMemo(() => {
    ...
    if (verifiedCount >= 2) return "CRITICAL";
  ...
  function completeMission(verifiedMap = verified) {
    ...
    setCompleted(true);
    setMissionStatus("MALWARE CONTAINED");
      ...
      {showCompleteAnim ? (
        <div className="complete-overlay blackout">
          <div className="complete-burst">
            ...
            <h2>STATUS · MALWARE CONTAINED</h2>
            <p className="mono">OPERATION BLACKOUT — PART III</p>
            <p className="continued">ENCRYPTED ARCHIVE DETECTED</p>
          </div>
        </div>
      ) : null}

      <div className="mission-app mission-002 mission-008">
        ...
        <section className="glass section priority-alert" style={{ padding: "1.25rem 1.3rem", marginTop: "1rem" }}>
          ...
          <p className="mono dim" style={{ marginBottom: "0.75rem", fontSize: "0.82rem" }}>
            {mission.subtitle} · Classification TOP SECRET · Safe simulation
          </p>
          ...
          {!started ? (
            <p className="mono dim" style={{ marginTop: "0.75rem", fontSize: "0.82rem" }}>
              Click ENTER CONTAINMENT MODE to open the malware analysis lab.
            </p>
          ) : (
        ...
        <div className="m2-layout">
          <div className="m2-main stack">
            <section className="section">
              <div className="row" style={{ justifyContent: "space-between", marginBottom: "0.85rem" }}>
                <div>
                  <p className="eyebrow">Malware Analysis Lab</p>
                  <h2 style={{ fontSize: "1.15rem" }}>Containment dashboard</h2>
                </div>
              ...
              {started && activeEv ? (
                <div className="glass evidence-stage" style={{ marginTop: "1rem", padding: "1.1rem" }}>
                  ...
                  {activeEv.type === "files" && <FileExplorer />}
                  {activeEv.type === "behavior" && <BehaviourReport />}
                  {activeEv.type === "systems" && <AffectedSystems />}
                  {activeEv.type === "av" && <AntivirusReport />}
                  {activeEv.type === "recovery" && <RecoveryPlan />}
                </div>
              ) : null}
            </section>
            ...
            {completed && completionMeta ? (
              <section className="glass" style={{ padding: "1.25rem 1.3rem" }}>
                ...
                <div className="continued-card">
                  <p className="eyebrow">Final alert</p>
                  <h3>⚠ The malware has been contained…</h3>
                  <p className="muted">
                    Investigators discover one final encrypted archive left behind by the attacker. No one knows what
                    it contains. Prepare for Mission 009.
                  </p>
                </div>
              </section>
            ) : null}
          ...
          <div className="toolkit-wrap">
            <div className="glass panel status-chip">
              ...
              <div className="mono" style={{ marginTop: "0.2rem" }}>
                {progress}%
              </div>
            </div>
